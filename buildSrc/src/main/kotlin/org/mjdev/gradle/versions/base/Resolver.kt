...
class Resolver(
    ...
    private fun createLatestConfiguration(
        ...
        if (asBoolean(
                getMetaClass(copy.resolutionStrategy).hasProperty(
                    copy.resolutionStrategy,
                    "failOnDynamicVersions"
                )
            )
        ) {
            getMetaClass(copy.resolutionStrategy).setProperty(
                copy.resolutionStrategy,
                "failOnDynamicVersions",
                false
            )
        ...
        val isKotlinDep = { dependency: ExternalDependency? ->
            val group = dependency?.group ?: ""
            (group.startsWith("org.jetbrains.kotlin"))
        }
        val inheritedKotlin = configuration
            .allDependencies
            .filterIsInstance<ExternalDependency>()
        ...
        val metadata = project
            .configurations
            .findByName("commonMainMetadataElements")
        if (metadata == null) {
            val compile = project
                .configurations
                .findByName("compile")
    ...
    private fun createQueryDependency(
        ...
        val nonTransitiveDependency = project.dependencies
            .create("${dependency.group.orEmpty()}:${dependency.name}:$version") as ModuleDependency
        nonTransitiveDependency.isTransitive = false
        return nonTransitiveDependency
    }

    private fun disableAutoTargetJvm(
        configuration: Configuration
    ) {
    ...
    private fun addRevisionFilter(
        ...
        configuration.resolutionStrategy {
            componentSelection {
                val revisionFilter =
                    { selection: ComponentSelection, metadata: ComponentMetadata? ->
                        ...
                        if (!accepted) {
                            selection
                                .reject("Component status ${metadata?.status} rejected by revision $revision")
                        }
                    }
                all {
                    if (ComponentSelection::class.members.any { it.name == "getMetadata" }) {
                        revisionFilter(this, metadata)
                    }
                }
            }
        }
    ...
    private fun addCustomResolutionStrategy(
        ...
        configuration.resolutionStrategy {
            resolutionStrategy?.execute(ResolutionStrategyWithCurrent(this, currentCoordinates))
        }
    }

    private fun getCurrentCoordinates(
        configuration: Configuration
    ): Map<Coordinate.Key, Coordinate> {
    ...
    private fun logRepositories() {
        ...
        val label = "${
            if (root) project.name else project.path
        } project ${if (root) " (root)" else ""}"
        if (
            !project
                .buildscript
                .configurations
    ...
    private fun supportsConstraints(
        configuration: Configuration
    ): Boolean = checkConstraints && !configuration.allDependencyConstraints.isEmpty()

    private fun getResolvableDependencies(
        configuration: Configuration
    ): List<Coordinate> {
